...
from libThresholdLogic import ConstNeuron, Perceptron, ProxyNeuron, NeuronNetwork
...
class GenericNumberAdder(NeuronNetwork):
    ...
    def __init__(self, n_bit: int, n_neurons: int) -> None:
        ...
        for carry_out_adder_idx, carry_out_adder in enumerate(bit_adders):
            ...
            for carry_in_neuron_idx, (carry_out_neuron, carry_in_adder) in enumerate(zip(
                carry_out_adder.carry_outputs, carry_in_adders
            )):
                # XXX having to manually access inputs by index, and rather carefully
                # could be simplified by using a `ProxyNeuronGroup`
                carry_in_adder.carry_inputs[carry_in_neuron_idx].source = carry_out_neuron
        ...
        for carry_in_adder_idx, carry_in_adder in enumerate(bit_adders[:bit_adder_n_carry_inputs]):
            for neuron_carry_in in carry_in_adder.carry_inputs[carry_in_adder_idx:]:
                neuron_carry_in.source = ConstNeuron(0.0)

        # arrange input layer such that the network accepts a sequence of little-bittian numbers
        input_layer = list(itertools.chain.from_iterable(
            (bit_adder.real_inputs[t] for bit_adder in bit_adders)
            for t in range(bit_adder_n_real_inputs)
        ))
